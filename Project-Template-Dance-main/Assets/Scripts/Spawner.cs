...
public class Spawner : MonoBehaviour
{
    ...
    /*    void SpawnArrow()
        {
            Vector3 leftSpawnPosition = leftSpawnPositions[currentSpawnIndex];
            Vector3 rightSpawnPosition = rightSpawnPositions[currentSpawnIndex];
            Vector3 spawnRotation = spawnRotations[currentRotationIndex];

            Quaternion rotation = Quaternion.Euler(spawnRotation);
            Instantiate(myPrefab, leftSpawnPosition, rotation);
            Instantiate(myPrefab, rightSpawnPosition, rotation);

            currentSpawnIndex = (currentSpawnIndex + 1) % leftSpawnPositions.Length;
            currentRotationIndex = (currentRotationIndex + 1) % spawnRotations.Length;
        }*/
    ...
    */
    ...
    /*    void SpawnArrow(string side)
        {
            ...
            {
                // Select a random index from the rightSpawnPositions array
                int randomIndex = Random.Range(0, rightSpawnPositions.Length);
                spawnPosition = rightSpawnPositions[randomIndex];
            }
    ...
    /*    void HandleArrowDestroyed(string side)
        {
            // Spawn a new arrow on the same side
            SpawnArrow(side);
        }*/
